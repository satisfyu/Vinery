...
public abstract class PrivateRecipeBookWidget extends GuiComponent implements PlaceRecipe<Ingredient>, Widget, GuiEventListener, RecipeShownListener {
	...
	static {
		SEARCH_HINT_TEXT = new TranslatableComponent("gui.recipebook.search_hint").withStyle(ChatFormatting.ITALIC)
				.withStyle(ChatFormatting.GRAY);
		TOGGLE_CRAFTABLE_RECIPES_TEXT = new TranslatableComponent("gui.recipebook.toggleRecipes.craftable");
	...
	@Nullable private PrivateRecipeGroupButtonWidget currentTab;
	...
	private void drawGhostSlotTooltip(PoseStack matrices, int x, int y, int mouseX, int mouseY) {
		...
		if (itemStack != null && this.client.screen != null) {
			this.client.screen.renderComponentTooltip(matrices, this.client.screen.getTooltipFromItem(itemStack),
					mouseX, mouseY);
		}
	}
	...
	private void refreshResults(boolean resetCurrentPage) {
		...
		List<? extends Recipe<Container>> recipes = getResultsForGroup(currentTab.getGroup(),
				client.level.getRecipeManager().getAllRecipesFor(getRecipeType()));
		String string = this.searchField.getValue();
		if (!string.isEmpty()) {
			recipes.removeIf((recipe) -> !recipe.getResultItem().getHoverName().getString().toLowerCase(Locale.ROOT)
	...
	public void reset() {
		...
		this.searchField = new EditBox(var10003, var10004, var10005, 80, 9 + 5,
		...
		this.toggleCraftableButton = new StateSwitchingButton(i + 110, j + 12, 26, 16,
				VineryClient.rememberedCraftableToggle);
		...
		if (this.currentTab != null) {
			this.currentTab = this.tabButtons.stream().filter(
					(button) -> button.getGroup().equals(this.currentTab.getGroup())).findFirst().orElse(null);
		}
		if (this.currentTab == null) {
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (this.open && !this.client.player.isSpectator()) {
			if (this.recipesArea.mouseClicked(mouseX, mouseY, button, (this.parentWidth - 147) / 2 - this.leftOffset,
					(this.parentHeight - 166) / 2, 147, 166)) {
	...
	private void triggerPrivateEasterEgg(String search) {
		...
		playerEntity.sendMessage(Component.nullToEmpty("<" + name + "> " + text), Util.NIL_UUID);
	...
	public boolean isClickOutsideBounds(double mouseX, double mouseY, int x, int y, int backgroundWidth, int backgroundHeight) {
		if (!this.isOpen()) {
			...
			boolean bl = mouseX < (double) x || mouseY < (double) y || mouseX >= (double) (x + backgroundWidth)
					|| mouseY >= (double) (y + backgroundHeight);
			boolean bl2 =
					(double) (x - 147) < mouseX && mouseX < (double) x && (double) y < mouseY && mouseY < (double) (y
							+ backgroundHeight);
	...
	private Component getCraftableButtonText() {
		return this.toggleCraftableButton.isStateTriggered() ? this.getToggleCraftableButtonText()
