...
public enum CookingPotRecipeBookGroup implements IRecipeBookGroup {
	SEARCH(new ItemStack(Items.COMPASS)), JAM(new ItemStack(BlockRegistry.CHERRY_JAM.get())), MISC(
	...
	public boolean fitRecipe(Recipe<?> recipe) {
		if (recipe instanceof CookingPotRecipe cookingPotRecipe) {
			switch (this) {
				...
				case JAM -> {
					if (cookingPotRecipe.getContainer().getItem() == BlockRegistry.CHERRY_JAR.get().asItem()) {
						return true;
					}
				}
				case MISC -> {
