...
public class PrivateRecipeAlternativesWidget extends GuiComponent implements Widget, GuiEventListener {
	...
	public void showAlternativesForResult(Minecraft client, Recipe<?> recipe, int buttonX, int buttonY, int areaCenterX, int areaCenterY, float delta) {
		...
		this.alternativeButtons.add(
				new CustomAlternativeButtonWidget(this.buttonX + 6, this.buttonY + 6, this.recipe, bl));
	...
	private void renderGrid(PoseStack matrices, int i, int j, int k, int l, int m, int n) {
		...
		for (int o = 0; o < i; ++o) {
			...
			for (int p = 0; p < j; ++p) {
				...
				this.blit(matrices, this.buttonX + l + (o + 1) * k - 1, this.buttonY + l + (p + 1) * k - 1, m + l,
						n + l, l + 1, l + 1);
				if (o == i - 1) {
					this.blit(matrices, this.buttonX + l * 2 + i * k, this.buttonY + l + p * k, m + k + l, n + l, l, k);
					this.blit(matrices, this.buttonX + l * 2 + i * k, this.buttonY + l + (p + 1) * k, m + k + l, n + l,
							l, l);
				}
			}
	...
	private class CustomAlternativeButtonWidget extends AbstractWidget implements PlaceRecipe<Ingredient> {
		...
		public CustomAlternativeButtonWidget(int x, int y, Recipe<?> recipe, boolean craftable) {
			super(x, y, 200, 20, Component.nullToEmpty(""));
			this.width = 24;
			this.height = 24;
			this.recipe = recipe;
		...
		public void renderButton(PoseStack matrices, int mouseX, int mouseY, float delta) {
			...
			for (InputSlot inputSlot : this.slots) {
				...
				PrivateRecipeAlternativesWidget.this.client.getItemRenderer().renderAndDecorateItem(
						inputSlot.stacks[Mth.floor(PrivateRecipeAlternativesWidget.this.time / 30.0F)
								% inputSlot.stacks.length], 0, 0);
				matrixStack.popPose();
			}
			matrixStack.popPose();
			RenderSystem.applyModelViewMatrix();
