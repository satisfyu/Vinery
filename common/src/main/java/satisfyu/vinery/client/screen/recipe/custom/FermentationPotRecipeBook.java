...
public class FermentationPotRecipeBook extends PrivateRecipeBookWidget {
	...
	static {
		TOGGLE_FERMENTABLE_TEXT = new TranslatableComponent("gui.vinery.recipebook.toggleRecipes.fermentable");
	}
	...
	public void showGhostRecipe(Recipe<?> recipe, List<Slot> slots) {
		...
		this.ghostSlots.addSlot(ItemRegistry.WINE_BOTTLE.getOrNull().asItem().getDefaultInstance(), slots.get(0).x,
				slots.get(0).y);
		int j = 1;
	...
	public void insertRecipe(Recipe<?> recipe) {
		if (recipe instanceof FermentationBarrelRecipe) {
			...
			for (Slot slot : screenHandler.slots) {
				if (slot.getItem().getItem() == ItemRegistry.WINE_BOTTLE.getOrNull().asItem()) {
					...
					Minecraft.getInstance().gameMode.handleInventoryMouseClick(screenHandler.containerId, 0, 0,
							ClickType.PICKUP, Minecraft.getInstance().player);
					break;
				}
				++slotIndex;
	...
	protected RecipeType<? extends Recipe<Container>> getRecipeType() {
		return VineryRecipeTypes.FERMENTATION_BARREL_RECIPE_TYPE.getOrNull();
	}
