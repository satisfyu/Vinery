...
public abstract class AbstractRecipeBookGUIScreenHandler extends AbstractPrivateRecipeScreenHandler {
	...
	public ItemStack quickMoveStack(Player player, int invSlot) {
		...
		if (slot.hasItem()) {
			...
			else if (invSlot >= entityInputStart && invSlot < entityOutputSlot ? !this.moveItemStackTo(itemStack2,
					inventoryStart, hotbarEnd, false) : !this.moveItemStackTo(itemStack2, entityInputStart,
					entityOutputSlot, false) && (invSlot >= inventoryStart && invSlot < hotbarStart
					? !this.moveItemStackTo(itemStack2, hotbarStart, hotbarEnd, false)
					: invSlot >= hotbarStart && invSlot < hotbarEnd && !this.moveItemStackTo(itemStack2, inventoryStart,
							hotbarStart, false))) {
				return ItemStack.EMPTY;
			}
