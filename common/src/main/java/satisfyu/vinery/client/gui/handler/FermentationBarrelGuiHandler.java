...
public class FermentationBarrelGuiHandler extends AbstractRecipeBookGUIScreenHandler {
	...
	public FermentationBarrelGuiHandler(int syncId, Inventory playerInventory, Container inventory, ContainerData propertyDelegate) {
		super(VineryScreenHandlerTypes.FERMENTATION_BARREL_GUI_HANDLER.getOrNull(), syncId, 5, playerInventory, inventory,
				propertyDelegate);
	...
	private void buildBlockEntityContainer(Inventory playerInventory, Container inventory) {
		// Wine input
		this.addSlot(new ExtendedSlot(inventory, 0, 79, 51, stack -> stack.is(ItemRegistry.WINE_BOTTLE.getOrNull())));
		// Inputs
	...
	private boolean isIngredient(ItemStack stack) {
		return this.world.getRecipeManager().getAllRecipesFor(VineryRecipeTypes.FERMENTATION_BARREL_RECIPE_TYPE.getOrNull())
