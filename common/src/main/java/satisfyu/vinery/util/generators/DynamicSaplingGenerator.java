...
public abstract class DynamicSaplingGenerator extends AbstractTreeGrower {
	...
	@Nullable
	protected abstract Holder<? extends ConfiguredFeature<?, ?>> getTreeFeature(ServerLevel world, Random random, boolean bees);

	public boolean growTree(ServerLevel world, ChunkGenerator chunkGenerator, BlockPos pos, BlockState state, Random random) {
		Holder<? extends ConfiguredFeature<?, ?>> registryEntry = this.getTreeFeature(world, random,
				this.hasFlowers(world, pos));
	...
	private boolean hasFlowers(LevelAccessor world, BlockPos pos) {
		for (BlockPos blockPos : BlockPos.MutableBlockPos.betweenClosed(pos.below().north(2).west(2),
