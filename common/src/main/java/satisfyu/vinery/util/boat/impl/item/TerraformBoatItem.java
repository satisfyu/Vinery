...
public class TerraformBoatItem extends Item {
	...
	/**
	 * @param boatSupplier a {@linkplain Supplier supplier} for the {@linkplain TerraformBoatType Terraform boat type}
	 * that should be spawned by this item
	 */
	public TerraformBoatItem(Supplier<TerraformBoatType> boatSupplier, Properties settings) {
		super(settings);
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player user, InteractionHand hand) {
		...
		List<Entity> riders = world.getEntities(user,
				user.getBoundingBox().expandTowards(rotationVec.scale(5d)).inflate(1d), RIDERS);
