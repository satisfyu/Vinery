...
public class TooltipHelper {
	...
	private static void wrapLongLines(List<ClientTooltipComponent> components, Font textRenderer, int maxSize) {
		for (int i = 0; i < components.size(); i++) {
			if (components.get(i) instanceof ClientTextTooltip orderedTextTooltipComponent) {
				Component text = OrderedTextToTextVisitor.get(
						((OrderedTextToolTipAccessor) orderedTextTooltipComponent).getText());
				List<ClientTooltipComponent> wrapped = textRenderer.split(text, maxSize).stream().map(
			...
			if (components.get(i) instanceof ClientTextTooltip orderedTextTooltipComponent) {
				...
				for (int j = 0; j < children.size() - 1; j++) {
					...
					if (code.equals("\\n")) {
						...
						components.add(i + 1, ClientTooltipComponent.create(
								textWithChildren(children, j + 2, children.size()).getVisualOrderText()));
						break;
					}
				}
			}
	...
	private static Component textWithChildren(List<Component> children, int from, int end) {
		MutableComponent text = Component.nullToEmpty("").copy();
		for (int i = from; i < end; i++) { text.append(children.get(i)); }
		return text;
	}
}