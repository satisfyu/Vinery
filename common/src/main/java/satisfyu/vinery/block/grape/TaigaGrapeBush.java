...
public class TaigaGrapeBush extends GrapeBush {
	...
	public boolean canGrowPlace(LevelReader world, BlockPos blockPos, BlockState blockState) {
		...
		int size = 4;
		Iterator<BlockPos> var2 = BlockPos.betweenClosed(blockPos.offset(-size, -2, -size),
				blockPos.offset(size, 1, size)).iterator();
		BlockPos pos;
		do {
			if (!var2.hasNext()) {
				return false;
		...
		while (!(world.getBlockState(pos).getBlock() == Blocks.PODZOL || world.getBlockState(pos).getBlock()
				== Blocks.COARSE_DIRT || world.getBlockState(pos).getBlock() == Blocks.GRASS_BLOCK));
		return true;
	}
	...
	public void entityInside(BlockState state, Level world, BlockPos pos, Entity entity) {
		if (entity instanceof LivingEntity && entity.getType() != EntityType.FOX
			...
			if (!world.isClientSide && state.getValue(AGE) > 0 && (entity.xOld != entity.getX()
