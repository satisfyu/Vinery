...
public class PaleStemBlock extends StemBlock {
	...
	public BlockState getStateForPlacement(BlockPlaceContext ctx) {
		...
		if (blockState.canSurvive(ctx.getLevel(), ctx.getClickedPos())) {
			...
			if (placeStack != null && (ctx.getPlayer().isCreative() || placeStack.getCount() >= 2)
					&& world.getBlockState(blockPos.below()).getBlock() != this
					&& blockPos.getY() < world.getMaxBuildHeight() - 1 && world.getBlockState(blockPos.above())
					.canBeReplaced(ctx)) {
	...
	public void appendHoverText(ItemStack itemStack, BlockGetter world, List<Component> tooltip, TooltipFlag tooltipContext) {
		tooltip.add(new TranslatableComponent("block.vinery.stem.tooltip").withStyle(ChatFormatting.ITALIC,
				ChatFormatting.GRAY));
		if (Screen.hasShiftDown()) {
			tooltip.add(new TranslatableComponent("item.vinery.stem2.tooltip"));
		}
		else {
