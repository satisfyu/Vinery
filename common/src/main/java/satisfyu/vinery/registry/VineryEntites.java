...
public class VineryEntites {
	private static final Registrar<EntityType<?>> ENTITY_TYPES = DeferredRegister.create(Vinery.MODID,
			Registry.ENTITY_TYPE_REGISTRY).getRegistrar();

	public static final RegistrySupplier<EntityType<TraderMuleEntity>> MULE = create("mule",
			() -> EntityType.Builder.of(TraderMuleEntity::new, MobCategory.CREATURE).sized(0.9f, 1.87f)
					.clientTrackingRange(10).build(new VineryIdentifier("mule").toString()));

	public static final RegistrySupplier<EntityType<WanderingWinemakerEntity>> WANDERING_WINEMAKER = create(
	...
	public static final RegistrySupplier<EntityType<ChairEntity>> CHAIR = create("chair", () -> EntityType.Builder.of(
					ChairEntity::new, MobCategory.MISC).sized(0.001F, 0.001F)
			.build(new ResourceLocation(Vinery.MODID, "chair").toString()));
