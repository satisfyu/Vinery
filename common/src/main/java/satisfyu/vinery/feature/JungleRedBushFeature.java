        ...
        if (!structureWorldAccess.isEmptyBlock(blockPos)) {
            ...
            for (Direction direction : directions) {
                if (direction != Direction.DOWN && GrapeVineBlock.isAcceptableNeighbour(structureWorldAccess, blockPos.relative(direction), direction)) {
                    structureWorldAccess.setBlock(blockPos, ObjectRegistry.JUNGLE_RED_GRAPE_BUSH.defaultBlockState().setValue(GrapeVineBlock.AGE, 2).setValue(GrapeVineBlock.getPropertyForFace(direction), true), 2);
                    return true;
                }
            }
